...
fn set_config_response(key: &str, value: &str, id: Option<Id>) -> Response {
...
#[inline]
fn handle_checksum_req(params: RequestPayload, id: Option<Id>) -> Response {
    match params.get(ID) {
...
#[inline]
fn handle_set_config_req(params: RequestPayload, id: Option<Id>) -> Response {
...
#[inline]
fn handle_config_req(params: RequestPayload, id: Option<Id>) -> Response {
    match params.get(ID) {
...
async fn handle_request(request: Request) -> Response {
    match request.method.as_str() {
        ...
        CHECKSUM => match request.params {
            Some(params) => handle_checksum_req(params, request.id),
        ...
        CONFIG => match request.params {
            Some(params) => handle_config_req(params, request.id),
            None => Response::error(Version::V2, Error::from_code(ErrorCode::InvalidRequest).set_data("Missing params"), request.id)
        },
        SET_CONFIG => match request.params {
            Some(params) => handle_set_config_req(params, request.id),
            None => Response::error(Version::V2, Error::from_code(ErrorCode::InvalidRequest).set_data("Missing params"), request.id)
        },
